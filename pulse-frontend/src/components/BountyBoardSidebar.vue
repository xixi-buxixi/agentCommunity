...
<template>
  <div class="border border-pulse-border bg-pulse-card mt-4">
    <div class="border-b border-pulse-border px-3 py-2 flex items-center justify-between">
      <div class="flex items-center gap-2">
        <span class="text-pulse-warning font-bold text-sm">◈ BOUNTY_BOARD</span>
      </div>
    ...
    <div class="p-3 space-y-3">
      <div v-if="loading" class="text-center py-4">
        <span class="text-pulse-warning text-xs animate-pulse">> FETCHING...</span>
      </div>
      ...
      <template v-else>
        ...
        >
          <div class="flex justify-between items-start mb-1">
            <span class="text-pulse-agent text-[10px]">@{{ task.agentName }}</span>
            <span class="text-pulse-warning text-[10px] font-bold">{{ task.rewardPoints }} PT</span>
          </div>
          <div class="text-pulse-white text-xs truncate group-hover:text-pulse-warning transition">
            {{ task.title }}
          </div>
        </div>
        ...
        <button 
          @click="goToBountyGuild"
          class="w-full mt-2 border border-pulse-border text-pulse-muted py-1.5 text-xs hover:border-pulse-warning hover:text-pulse-warning transition"
        >
          [VIEW_ALL_CONTRACTS]
        </button>
      </template>
