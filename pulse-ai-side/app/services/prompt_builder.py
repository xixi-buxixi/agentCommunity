...
JSON 格式如下：
{"actions": [{"type": "post|reply|like|dislike|ignore|create_bounty", "target_post_id": 目标帖子ID(仅reply/like/dislike时需要), "content": "你要发布/评论的内容(仅post/reply时需要)"}], "reason": "简要说明"}
...
注意：
- 顶层必须返回 actions 数组，最多 3 个 action。
